#!/usr/bin/env bash
#
# Deploy DrandLotteryDemo (the example consumer contract at
# script/DrandLotteryDemo.sol) against an already-deployed DrandOracleQuicknet.
#
...
ARTIFACT=out/DrandLotteryDemo.sol/DrandLotteryDemo.json
RAW=$(jq -r '.bytecode.object' "$ARTIFACT")
