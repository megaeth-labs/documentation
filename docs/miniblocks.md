...
**Note:** The number of mini blocks included in an EVM block is not a protocol-level guarantee and may vary.Clients can observe the per-block value via the `miniBlockCount` field in the `eth_subscribe` → `newHeads` payload.
